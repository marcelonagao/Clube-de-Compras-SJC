...
const initialProducts = [
  {
    ...
    sku: 'HORT-001',
    category: 'Hortifruti',
    name: 'Tomate Orgânico (kg)',
    description:
      'Tomates frescos colhidos no dia. Excelentes para saladas e molhos.',
    price: 8.5,
    minOrderQuantity: 20,
    stockLocal: 5,
    image: '🍅',
  },
  {
    ...
    description: 'Maço grande de alface crespa hidropônica.',
    price: 3.0,
    minOrderQuantity: 30,
    stockLocal: 0,
    image: '🥬',
  },
  {
    ...
    description: 'Arroz branco tipo 1, safra nova.',
    price: 24.5,
    minOrderQuantity: 10,
    stockLocal: 2,
    image: '🍚',
  },
  {
    ...
    description: 'Peito de frango resfriado sem osso e sem pele.',
    price: 19.9,
    minOrderQuantity: 15,
    stockLocal: 0,
    image: '🍗',
  },
  {
    ...
    sku: 'CARN-002',
    category: 'Carnes & Aves',
    name: 'Coração de Frango (kg)',
    description: 'Coração de frango limpo e resfriado.',
    price: 22.5,
    minOrderQuantity: 10,
    stockLocal: 0,
    image: '❤️',
  },
  {
    ...
    description: 'Mel puro de abelhas silvestres da região.',
    price: 35.0,
    minOrderQuantity: 12,
    stockLocal: 1,
    image: '🍯',
...
const initialOrders = [
  {
    ...
    items: [{ id: 1, name: 'Tomate Orgânico (kg)', qtd: 3 }],
  },
  {
    id: '1002',
    customer: 'Dona Maria (Via WhatsApp)',
    polo: 'Jacareí',
    total: 15.0,
    method: 'dinheiro/pix direto',
    status: 'pago',
    date: '2026-02-28T10:00:00.000Z',
    items: [{ id: 2, name: 'Alface Crespa (un)', qtd: 5 }],
  },
];
...
export default function App() {
  ...
  const addToManualCart = (product) => {
    ...
    if (existing) {
      ...
      setManualCart([...manualCart, { ...product, qtd: 1 }]);
    }
  };

  const manualCartTotal = manualCart.reduce(
    (sum, item) => sum + item.price * item.qtd,
    0
  );

  const confirmManualOrder = (e) => {
    e.preventDefault();
    if (manualCart.length === 0)
      return showToast('Por favor, adicione produtos ao pedido!', 'error');

    const existingCustomer = customers.find(
      (c) =>
        c.whatsapp === manualCustomerWhatsapp || c.email === manualCustomerEmail
    );
    ...
    const newOrder = {
      ...
      customer: `${manualCustomerName} (Via WhatsApp)`,
      email: manualCustomerEmail,
      whatsapp: manualCustomerWhatsapp,
      polo: user.polo,
    ...
    setManualCustomerWhatsapp('');
    setManualCart([]);
    showToast('Pedido lançado com sucesso! Notificação enviada.', 'success');
  };
  ...
  const downloadCSVTemplate = () => {
    const headers =
      'SKU,Categoria,Nome,Descricao,Preco,QtdMinimaFornecedor,EstoqueAtual,Imagem_URL_ou_Emoji\n';
    const sample =
      'HORT-001,Hortifruti,Tomate Orgânico (kg),Tomates frescos colhidos no dia.,8.50,20,5,🍅\n';
    ...
    link.setAttribute('download', 'modelo_catalogo_importacao.csv');
  ...
  const handleCSVUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    showToast(`Tabela "${file.name}" carregada com sucesso!`, 'success');
    e.target.value = null;
  };

  const deleteProduct = (id) => {
    if (window.confirm('Remover este produto do catálogo?')) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-green-50">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md text-center">
        ...
        <div className="space-y-3">
          <button
            onClick={() => {
              if (!loginName || !loginEmail || !loginWhatsapp)
                return showToast(
                  'Por favor, preencha todos os campos do Cliente!',
                  'error'
                );
              setUser({
                name: loginName,
            ...
            className="w-full flex items-center justify-center bg-green-100 text-green-700 font-bold py-3 rounded-xl hover:bg-green-200 transition"
          >
          ...
          <button
            onClick={() => {
              if (!loginName)
                return showToast(
                  'Por favor, digite o seu nome para entrar como Representante!',
                  'error'
                );
              setUser({
                name: loginName,
              ...
              setCurrentScreen('dashboard_rep');
            }}
            className="w-full flex items-center justify-center bg-blue-100 text-blue-700 font-bold py-3 rounded-xl hover:bg-blue-200 transition"
          >
            ...
            <button
              onClick={() => {
                setUser({
                  name: 'Você (Gestor)',
                  role: 'consolidador',
                  polo: 'Todas as Cidades',
                });
                setCurrentScreen('dashboard_admin');
              }}
              className="w-full flex items-center justify-center bg-gray-800 text-white font-bold py-3 rounded-xl hover:bg-gray-900 transition shadow-lg"
            >
      ...
      <div className="pb-24 pt-4 px-4 max-w-4xl mx-auto">
        <div className="bg-green-100 text-green-800 p-4 rounded-xl mb-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center">
            ...
            <span>
              Retirada: <strong>{user.polo}</strong>
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProducts.map((product) => {
            ...
            const isImageUrl = product.image.length > 5;
              ...
              >
                <div className="flex items-start space-x-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xl text-green-600 font-black">
                ...
                {cartItem ? (
                  <div className="flex items-center justify-between w-full bg-green-50 border border-green-200 rounded-xl p-1">
                      ...
                      onClick={() =>
                        setCart(
                          cart
                            .map((i) =>
                              i.id === product.id
                                ? { ...i, qtd: Math.max(0, i.qtd - 1) }
                                : i
                            )
                            .filter((i) => i.qtd > 0)
                        )
                      }
                      className="w-10 h-10 flex items-center justify-center bg-white rounded-lg shadow-sm text-gray-600 font-bold hover:bg-gray-50"
                    >
                      -
                    </button>
                    ...
                    <button
                      ...
                      className="w-10 h-10 flex items-center justify-center bg-white rounded-lg shadow-sm text-green-600 font-bold hover:bg-gray-50"
                    >
                      +
                    </button>
                  </div>
                  ...
                  <button
                    onClick={() => addToCart(product)}
                    className="w-full bg-gray-50 text-gray-700 py-3 rounded-xl font-bold hover:bg-green-600 hover:text-white transition border border-gray-200"
                  >
                    Adicionar à Cesta
                  </button>
                )}
      ...
      <div className="p-4 max-w-xl mx-auto pb-24 pt-6">
        <div className="flex items-center mb-6">
          <button
            ...
            className="mr-4 flex items-center text-green-700 bg-green-100 px-3 py-2 rounded-xl font-bold hover:bg-green-200 transition text-sm shadow-sm"
          >
            <Edit2 className="w-4 h-4 mr-2" /> Editar Cesta
          </button>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 mb-6">
          ...
          {cart.map((item) => (
            <div
              key={item.id}
              className="flex justify-between text-sm mb-2 text-gray-600"
            >
              <span>
          ...
          <div className="border-t border-gray-100 mt-4 pt-4">
            ...
            {hasFee ? (
              <div className="flex justify-between text-orange-500 mb-2 text-sm font-bold">
                <span>Taxa da Maquininha (5%)</span>
                <span>+ R$ {feeAmount.toFixed(2).replace('.', ',')}</span>
              </div>
            ) : (
              <div className="flex justify-between text-green-500 mb-2 text-sm font-bold bg-green-50 p-2 rounded-lg">
                <span>Desconto PIX</span>
                <span>Aplicado! (Sem taxas)</span>
              </div>
            )}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-green-600 font-bold">
                Sem taxas extras (Sua melhor opção)
              </p>
            </div>
          </label>
          <label
        ...
        <button
          ...
          className={`w-full text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center transition ${
            isProcessingPayment
  ...
  const renderMyOrders = () => {
    const myOrders = orders.filter(
      (o) => o.customer === user.name && o.email === user.email
    );
    return (
      <div className="p-4 max-w-4xl mx-auto pt-6 pb-24">
        <div className="flex items-center mb-6">
          <button
            ...
            className="mr-4 flex items-center text-green-700 bg-green-100 px-3 py-2 rounded-xl font-bold hover:bg-green-200 transition text-sm shadow-sm"
          >
            <ArrowLeft className="w-4 h-4 mr-1" /> Loja
          </button>
        ...
        {myOrders.length === 0 ? (
          <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 text-center">
            ...
            <button
              onClick={() => setCurrentScreen('shop')}
              className="mt-4 bg-green-100 text-green-700 px-6 py-2 rounded-lg font-bold hover:bg-green-200 transition"
            >
              Ir às Compras
            </button>
          </div>
                ...
                >
                  <div className="flex justify-between items-center mb-3 border-b border-gray-50 pb-3">
                    ...
                    <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold">
                      {order.status === 'pago' ? 'Confirmado' : order.status}
                    </span>
                  </div>
  ...
  const renderRepDashboard = () => {
    const myPoloOrders = orders.filter(
      (o) => o.polo === user.polo && o.status === 'pago'
    );
    const appOrders = myPoloOrders.filter(
      (o) => o.method !== 'dinheiro/pix direto'
    );
    const manualOrders = myPoloOrders.filter(
      (o) => o.method === 'dinheiro/pix direto'
    );
    ...
    const ordersByMonth = myPoloOrders.reduce((acc, order) => {
      ...
      if (!acc[capitalizedMonth])
        acc[capitalizedMonth] = { orders: [], total: 0, count: 0 };
      ...
      <div className="p-4 max-w-4xl mx-auto pt-6 pb-24">
        <div className="mb-6">
          ...
          <p className="text-gray-500">
            Unidade: <strong className="text-blue-600">{user.polo}</strong>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center relative overflow-hidden">
            ...
            <span className="text-sm text-green-600 font-bold mt-1">
              R$ {sumTotal(appOrders).toFixed(2).replace('.', ',')}
            </span>
          </div>
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center relative overflow-hidden">
            <span className="text-xs text-gray-500 font-bold uppercase mb-1">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          <button
            ...
            className="flex-1 bg-green-50 text-green-700 border border-green-200 font-bold py-4 rounded-xl flex items-center justify-center hover:bg-green-100 transition"
          >
            <Plus className="w-5 h-5 mr-2" /> Lançar Pedido (WhatsApp)
          </button>
          <button
            onClick={() => setCurrentScreen('print_rep')}
            className="flex-1 bg-gray-800 text-white font-bold py-4 rounded-xl flex items-center justify-center hover:bg-gray-900 transition shadow-lg"
          >
            <Printer className="w-5 h-5 mr-2" /> Imprimir Separação (PDF)
          </button>
        </div>

        <h3 className="font-bold text-gray-700 mb-4 flex items-center">
          <ClipboardList className="w-5 h-5 mr-2 text-blue-500" /> Histórico por
          Mês
        </h3>

        <div className="space-y-4">
          {Object.entries(ordersByMonth).length === 0 ? (
            <p className="text-gray-500 text-center py-8 bg-white rounded-xl border border-gray-100">
                      ...
                      <div className="divide-y divide-gray-50">
                        {data.orders
                          ...
                          .map((order) => (
                            <div
                              key={order.id}
                              className="p-4 flex flex-col md:flex-row md:items-center justify-between hover:bg-gray-50"
                            >
                              <div>
                                <p className="font-bold text-gray-800">
                                  {order.customer}
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {order.items.map((item, idx) => (
                                    <span
                                      key={idx}
                                      className="bg-gray-100 text-gray-600 text-[10px] px-2 py-1 rounded border border-gray-200 uppercase font-bold"
                                    >
                              ...
                              <div className="mt-3 md:mt-0 flex flex-col items-end">
                                <span
                                  className={`px-2 py-1 rounded text-[10px] font-bold mb-2 ${
                                    order.method === 'dinheiro/pix direto'
            ...
            <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={confirmManualOrder}>
                <div className="mb-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
                  ...
                  <select
                    className="w-full border border-gray-300 rounded p-2 text-sm bg-white focus:ring-2 focus:ring-green-500 outline-none"
                    onChange={(e) => {
                      const c = customers.find((x) => x.id === e.target.value);
                      if (c) {
                        setManualCustomerName(c.name);
                        setManualCustomerEmail(c.email);
                        setManualCustomerWhatsapp(c.whatsapp);
                      }
                    }}
                  >
                    <option value="">
                      -- Selecionar da base (Novo Cliente) --
                    </option>
                    {customers
                      .filter((c) => c.polo === user.polo)
                      .map((c) => (
                  ...
                  {customers.filter((c) => c.polo === user.polo).length ===
                    0 && (
                    <p className="text-xs text-orange-500 mt-1 mb-2 font-medium">
                      Nenhum cliente salvo na base de {user.polo} ainda.
                    </p>
                  )}
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded p-2 text-sm mb-2 focus:ring-2 focus:ring-green-500 outline-none"
                  />
                  ...
                  <div className="flex gap-2 mb-2">
                    <div className="flex-1">
                      ...
                      <input
                        type="tel"
                        value={manualCustomerWhatsapp}
                        onChange={(e) =>
                          setManualCustomerWhatsapp(e.target.value)
                        }
                ...
                <div className="space-y-2 mb-4 max-h-48 overflow-y-auto border border-gray-100 rounded-lg p-2">
                  {products.map((p) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center">
                          <span className="text-2xl mr-2">{p.image}</span>
                        ...
                        {mItem ? (
                          <div className="flex items-center space-x-2 bg-white rounded border px-2 py-1 shadow-sm">
                            <button
                              ...
                              className="w-6 h-6 flex items-center justify-center font-bold text-gray-500 hover:bg-gray-100 rounded"
                            >
                              -
                            </button>
                            ...
                            <button
                              ...
                              className="w-6 h-6 flex items-center justify-center font-bold text-green-600 hover:bg-green-50 rounded"
                            >
                              +
                            </button>
                          </div>
                          ...
                          <button
                            type="button"
                            onClick={() => addToManualCart(p)}
                            className="text-sm bg-green-100 text-green-700 px-3 py-1 rounded-lg font-bold hover:bg-green-200"
                          >
                            Add
                          </button>
                        )}
                ...
                <div className="flex justify-between items-center border-t border-gray-200 pt-4 mb-6">
                  ...
                  <span className="text-xl font-black text-green-600">
                    R$ {manualCartTotal.toFixed(2).replace('.', ',')}
                  </span>
                </div>
  ...
  const renderPrintView = () => {
    const myPoloOrders = orders
      .filter((o) => o.polo === user.polo && o.status === 'pago')
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 sm:p-8">
        <div className="max-w-4xl mx-auto mb-6 flex justify-between items-center print:hidden bg-white p-4 rounded-xl shadow-sm border border-gray-200">
          <button
            onClick={() => setCurrentScreen('dashboard_rep')}
            className="flex items-center text-gray-600 hover:text-gray-800 font-bold px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200 transition"
          >
            <ArrowLeft className="w-5 h-5 mr-2" /> Voltar
          </button>
          <button
            onClick={() => window.print()}
            className="flex items-center bg-blue-600 text-white font-bold px-6 py-3 rounded-lg hover:bg-blue-700 shadow transition"
          >
        ...
        <div className="max-w-4xl mx-auto bg-white p-8 sm:p-12 shadow-md print:shadow-none print:p-0">
          <div className="text-center border-b-2 border-gray-800 pb-6 mb-8">
            ...
            <p className="text-lg text-gray-600 mt-2">
              Unidade: <strong>{user.polo}</strong>
            </p>
          </div>
          <div className="space-y-8">
            {myPoloOrders.length === 0 ? (
              <p className="text-center text-gray-500 italic">
                Não há pedidos para separar.
              </p>
            ) : (
              myPoloOrders.map((order) => (
                <div
                  key={order.id}
                  className="break-inside-avoid border border-gray-300 rounded-lg p-6"
                >
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="text-xs uppercase text-gray-500 border-b border-gray-100">
                        <th className="pb-2 w-16">Qtd</th>
                        <th className="pb-2">Produto</th>
                        <th className="pb-2 text-right">Verificado</th>
                      </tr>
                    </thead>
                    <tbody>
                      {order.items.map((item, idx) => (
                        ...
                        >
                          <td className="py-2 font-black text-lg">
                            {item.qtd}x
                          </td>
  ...
  const renderAdminPrintView = () => {
    ...
    paidOrders.forEach((order) => {
      ...
      order.items.forEach((item) => {
        if (!itemsByPolo[order.polo][item.id])
          itemsByPolo[order.polo][item.id] = {
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 sm:p-8">
        <div className="max-w-4xl mx-auto mb-6 flex justify-between items-center print:hidden bg-white p-4 rounded-xl shadow-sm border border-gray-200">
          <button
            onClick={() => setCurrentScreen('dashboard_admin')}
            className="flex items-center text-gray-600 hover:text-gray-800 font-bold px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200 transition"
          >
            <ArrowLeft className="w-5 h-5 mr-2" /> Voltar
          </button>
          <button
            ...
            className="flex items-center bg-blue-600 text-white font-bold px-6 py-3 rounded-lg hover:bg-blue-700 shadow transition"
          >
            <Printer className="w-5 h-5 mr-2" /> Imprimir Relatório de Despacho
          </button>
        </div>
        <div className="max-w-4xl mx-auto bg-white p-8 sm:p-12 shadow-md print:shadow-none print:p-0">
          <div className="text-center border-b-2 border-gray-800 pb-6 mb-8">
          ...
          <div className="space-y-12">
            {Object.entries(itemsByPolo).map(([polo, items]) => (
              <div
                key={polo}
                className="break-inside-avoid border-2 border-gray-800 p-6 rounded-lg relative mt-4"
              >
                <h2 className="absolute -top-4 left-4 px-4 text-xl font-black tracking-widest bg-gray-800 text-white py-1 rounded">
                  Destino: {polo}
                </h2>
                <table className="w-full text-left mt-4">
                  <thead>
                    <tr className="text-xs uppercase text-gray-500 border-b border-gray-300">
                      <th className="pb-2 w-20">Qtd Enviar</th>
                      <th className="pb-2">Produto</th>
                      <th className="pb-2">SKU</th>
                      <th className="pb-2 text-right">Conferido</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Object.values(items)
                      ...
                      .map((item) => (
                        ...
                        >
                          <td className="py-3 font-black text-xl text-blue-700">
                            {item.totalQtd}x
                          </td>
                          <td className="py-3 text-gray-800 font-bold text-lg">
                            {item.name}
                          </td>
  ...
  const renderAdminDashboard = () => {
    ...
    paidOrders.forEach((order) => {
      order.items.forEach((item) => {
        ...
        if (!consolidatedItems[item.id].polos[order.polo])
          consolidatedItems[item.id].polos[order.polo] = 0;
    ...
    const downloadPurchaseOrderCSV = () => {
      let csvContent =
        'SKU,Produto,Qtd Vendida,Estoque Local Atual,Caixas para Comprar (Fornecedor),Tamanho da Caixa,Qtd Total a Receber\n';
      ...
      Object.values(consolidatedItems).forEach((prod) => {
        ...
        const totalVendidos = prod.totalQtd;
        const necessidadeExterna = Math.max(
          0,
          totalVendidos - estoqueLocalAtual
        );
        ...
        if (caixasParaComprar > 0) {
          ...
          csvContent += `${prod.sku},${
            prod.name
          },${totalVendidos},${estoqueLocalAtual},${caixasParaComprar},${moq},${
            caixasParaComprar * moq
          }\n`;
        }
      });
      ...
      if (!hasItemsToBuy)
        return showToast(
          'O estoque atual atende a demanda. Nenhuma caixa precisa ser comprada.',
          'success'
        );
      ...
      <div className="p-4 max-w-5xl mx-auto pt-6 pb-24">
        <div className="flex flex-wrap gap-2 mb-6 bg-gray-200 p-1 rounded-xl">
          <button
            ...
            className={`flex-1 py-2 px-2 rounded-lg font-bold text-sm transition ${
              adminTab === 'pedidos'
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-gray-800 p-4 font-bold text-white flex flex-col sm:flex-row justify-between sm:items-center gap-3">
              <span className="flex items-center">
                <Package className="w-5 h-5 mr-2" /> Inteligência de Compras
              </span>
              <div className="flex flex-wrap items-center gap-2">
                <button
                  onClick={downloadPurchaseOrderCSV}
                  className="flex items-center bg-green-600 text-white px-3 py-1.5 rounded-lg hover:bg-green-700 transition text-sm whitespace-nowrap"
                >
                  <Download className="w-4 h-4 mr-1" /> Pedido Fornecedor (CSV)
                </button>
                <button
                  onClick={() => setCurrentScreen('print_admin')}
                  className="flex items-center bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700 transition text-sm whitespace-nowrap"
                >
                  <Printer className="w-4 h-4 mr-1" /> Despacho (PDF)
                </button>
              </div>
            </div>
            <div className="divide-y divide-gray-100">
              {Object.values(consolidatedItems).length === 0 && (
              ...
              {Object.values(consolidatedItems).map((prod) => {
                ...
                const novoEstoqueLocal =
                  estoqueLocalAtual - totalVendidos + totalCompradoDoFornecedor;
                const isImageUrl = prod.image && prod.image.length > 5;

                return (
                  <div
                    key={prod.id}
                    className="p-5 hover:bg-gray-50 transition border-l-4 border-transparent hover:border-blue-500"
                  >
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-3 gap-4">
                      <div className="flex items-center mb-3 md:mb-0 lg:w-1/3">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded inline-block mt-1 border border-indigo-100">
                            Caixa Fornecedor: {moq} un
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 flex gap-2">
                        ...
                        <div className="flex-1 bg-blue-50 border border-blue-200 rounded-xl p-3 shadow-sm text-center relative">
                          <p className="text-xs text-blue-700 font-bold uppercase mb-1 flex justify-center items-center">
                            Pedir
                          </p>
                          <p className="text-2xl font-black text-blue-700">
                            {caixasParaComprar} Cx
                          </p>
                          {caixasParaComprar > 0 && (
                            <span className="absolute -top-2 -right-2 bg-blue-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded shadow">
                              +{totalCompradoDoFornecedor} un
                            </span>
                          )}
                        </div>
                        <div className="flex-1 bg-green-50 border border-green-200 rounded-xl p-3 shadow-sm text-center">
                          <p className="text-xs text-green-700 font-bold uppercase mb-1 flex justify-center items-center">
                            Sobra
                          </p>
        ...
        {adminTab === 'crm' && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-blue-600 p-4 font-bold text-white flex justify-between items-center">
              ...
              <span className="bg-blue-800 px-3 py-1 rounded-lg text-sm border border-blue-500">
                  ...
                  {customers
                    ...
                    .map((c) => (
                      <tr key={c.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="p-4 text-sm text-gray-600">{c.polo}</td>
            ...
            <div className="bg-blue-50 border border-blue-200 p-6 rounded-xl flex flex-col md:flex-row items-center justify-between shadow-sm">
              <div className="mb-4 md:mb-0">
                <h3 className="font-bold text-blue-800 flex items-center">
                  <FileSpreadsheet className="w-5 h-5 mr-2" /> Atualização
                  Mensal via Planilha
                </h3>
                <button
                  onClick={downloadCSVTemplate}
                  className="text-xs mt-2 font-bold text-blue-700 underline hover:text-blue-900 flex items-center"
                >
                  <Download className="w-3 h-3 mr-1" /> Baixar Modelo CSV
                </button>
              </div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-green-500 outline-none uppercase"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      name="category"
                      defaultValue={editingProduct?.category || categorias[1]}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 mb-1 text-orange-600">
                        Estoque Local
                      </label>
                      <input
                        name="stockLocal"
                        type="number"
                        min="0"
                        defaultValue={editingProduct?.stockLocal || 0}
                        required
                        className="w-full border border-orange-300 bg-orange-50 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 outline-none font-bold"
                      />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1 text-blue-600">
                      Qtd Mínima (Caixa)
                    </label>
                    <input
                      ...
                      required
                      className="w-full border border-blue-300 bg-blue-50 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 outline-none font-bold"
                    />
                  </div>
                </div>
                <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-gray-100">
                  ...
                  <button
                    type="submit"
                    className="px-8 py-3 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 shadow-md"
                  >
                    Salvar
                  </button>
                </div>
              ...
              <div className="divide-y divide-gray-100">
                {products.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center">
                        <div className="w-10 h-10 bg-gray-100 rounded border border-gray-200 flex items-center justify-center mr-4 overflow-hidden">
                          {isImageUrl ? (
                            <img
                              src={p.image}
                              alt=""
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-500">
                            Caixa: {p.minOrderQuantity} un | Estoque Local:{' '}
                            {p.stockLocal} un
                          </p>
                        </div>
                      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans relative">
      {toast && (
        <div
          className={`fixed top-5 left-1/2 transform -translate-x-1/2 z-[100] px-6 py-3 rounded-full shadow-2xl font-bold text-white transition-all flex items-center ${
            ...
            <div className="flex items-center space-x-3">
              {user?.role === 'cliente' && (
                <button
                  onClick={() =>
                    setCurrentScreen(
                      currentScreen === 'my_orders' ? 'shop' : 'my_orders'
                    )
                  }
                  className="text-xs bg-green-50 text-green-700 border border-green-200 px-3 py-1.5 rounded-lg font-bold hover:bg-green-100 transition"
                >
              ...
              <button
                ...
                className="text-xs bg-red-50 text-red-600 px-3 py-1 rounded-full font-bold hover:bg-red-100"
              >
                Sair
              </button>
            </div>
      ...
      <main>
        ...
        {currentScreen === 'success' && (
          <div className="flex flex-col items-center justify-center min-h-[70vh] p-4 text-center">
            ...
            <button
              onClick={() => setCurrentScreen('shop')}
              className="bg-green-600 text-white font-bold py-3 px-8 rounded-xl hover:bg-green-700 shadow-lg mt-4"
            >
              Voltar para a Loja
            </button>
          </div>
            ...
            <button
              onClick={() =>
                cart.length > 0
                  ...
                  : showToast(
                      'Sua cesta está vazia! Adicione produtos primeiro.',
                      'error'
                    )
              }
              className={`w-full flex items-center justify-center py-3 rounded-xl font-bold transition relative ${
                cart.length > 0
